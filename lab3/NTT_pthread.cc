...
void NTT(int *a,int opt,int p, int thread_num = 4)
{
    //蝴蝶变换
	for(int i = 0;i < len;++ i) if(i < rev[i]) std::swap(a[i],a[rev[i]]);
    //NTT主体
    int GINV = getinv(G,p);
    //固定大小数组效率高 && 大多数主流 CPU 的逻辑核心数 ≤ 64 && 其余元素未使用，不会产生副作用。
	...
	for(int i = 1;i < len;i <<= 1)
	{
        ...
        for (int t = 0; t < thread_num; ++t) {
            args[t] = {a, i, len, p, w, t, thread_num};
