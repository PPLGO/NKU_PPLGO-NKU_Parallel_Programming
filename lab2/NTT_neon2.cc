		...
		for(int j = 0,s = i << 1;j < len;j += s)
        {
            ...
            if(i < 4){
                ...
                for(int k = 0; k < i; k += 4)
                {
                    ...
                    // Y = mi * B
                    uint32_t y0 = mulmod(a[j+k+i+0], mi[0], p);
                    uint32_t y1 = mulmod(a[j+k+i+1], mi[1], p);
                    uint32_t y2 = mulmod(a[j+k+i+2], mi[2], p);
                    uint32_t y3 = mulmod(a[j+k+i+3], mi[3], p);
                    ...
                    uint32_t mi_vec0 = mulmod(mi[0], w4, p);
                    uint32_t mi_vec1 = mulmod(mi[1], w4, p);
                    uint32_t mi_vec2 = mulmod(mi[2], w4, p);
                    uint32_t mi_vec3 = mulmod(mi[3], w4, p);
