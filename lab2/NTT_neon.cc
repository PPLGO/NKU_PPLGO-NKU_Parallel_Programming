...
inline uint32_t mulmod(uint32_t a, uint32_t b, uint32_t p) {
		...
		for(int j = 0,s = i << 1;j < len;j += s)
        {
            ...
            if(i < 4){
                ...
                for(int k = 0; k < i; k += 4)
                {
                    ...
                    // Y = mi * B
                    uint32_t y0 = mulmod(vgetq_lane_u32(B, 0), vgetq_lane_u32(mi_vec, 0), p);
                    uint32_t y1 = mulmod(vgetq_lane_u32(B, 1), vgetq_lane_u32(mi_vec, 1), p);
                    uint32_t y2 = mulmod(vgetq_lane_u32(B, 2), vgetq_lane_u32(mi_vec, 2), p);
                    uint32_t y3 = mulmod(vgetq_lane_u32(B, 3), vgetq_lane_u32(mi_vec, 3), p);

                    uint32_t Y_tmp[4] = {y0, y1, y2, y3};
                    uint32x4_t Y = vld1q_u32(Y_tmp);            
                    ...
                    uint32_t mi_vec0 = mulmod(vgetq_lane_u32(mi_vec, 0), vgetq_lane_u32(mi_base, 0), p);
                    uint32_t mi_vec1 = mulmod(vgetq_lane_u32(mi_vec, 1), vgetq_lane_u32(mi_base, 1), p);
                    uint32_t mi_vec2 = mulmod(vgetq_lane_u32(mi_vec, 2), vgetq_lane_u32(mi_base, 2), p);
                    uint32_t mi_vec3 = mulmod(vgetq_lane_u32(mi_vec, 3), vgetq_lane_u32(mi_base, 3), p);   
                    uint32_t mi_tmp[4] = {mi_vec0, mi_vec1, mi_vec2, mi_vec3};
                    mi_vec = vld1q_u32(mi_tmp);           
                }
		    }
	    }
    }
