...
// 中国剩余定理的多项式乘法
void CRT_poly_multiply(LL *a, LL *b, LL *ab, int n, LL p, int rank, int size){
    len = 1, l = -1;
	while(len < n+n-1) len <<= 1,l++;
	for(int i = 0;i < len;++ i) rev[i] = (rev[i >> 1] >> 1) | ((i & 1) << l);
    for(int j = 0; j < len; ++j){
    ...
    //发送结果到主进程
    if(rank > 0)
        MPI_Send(ans[rank], len, MPI_LONG_LONG, 0, rank, MPI_COMM_WORLD);
    //接收其他进程的结果
    if(rank == 0){
        for(int i = 1; i < 4; ++i){
            MPI_Recv(ans[i], len, MPI_LONG_LONG, i, i, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
        }
    }
    //等待发送和接收完成
